package com.example.csc_mvvm.data.dto.branch
...
fun List<*>.toBranchesResponse(): List<BranchResponse> {
    val branches = mutableListOf<BranchResponse>()
    forEach {
        if (it is BranchResponse) branches.add(it)
    }
    return branches
}